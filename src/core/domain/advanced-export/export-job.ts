...
export class ExportJob extends ValueObject<ExportJobProps> {
  private constructor(props: ExportJobProps) { super(props); }
  static create(props: ExportJobProps): ExportJob { return new ExportJob(props); }

  get id(): ExportJobId { return this.props.id; }
  get templateId(): string { return this.props.templateId; }
  get portfolioId(): string { return this.props.portfolioId; }
  ...
  get sizeBytes(): number | undefined { return this.props.sizeBytes; }
  ...
  markCompleted(fileUrl: string, checksum: string, sizeBytes: number): ExportJob {
    return ExportJob.create({
      ...this.props, status: "COMPLETED", fileUrl, checksum, sizeBytes, completedAt: new Date(),
    });
  }
