...
describe("SellEvent", () => {
  describe("creation", () => {
    it("creates a sell event with given properties", () => {
      const event = new SellEvent("portfolio-1", "corr-1", "PETR4", 50, 30.00);
      expect(event.aggregateId).toBe("portfolio-1");
      expect(event.correlationId).toBe("corr-1");
      expect(event.assetId).toBe("PETR4");
      expect(event.quantity).toBe(50);
      expect(event.price).toBe(30.00);
    });

    it("sets type to SELL", () => {
      const event = new SellEvent("p1", "c1", "PETR4", 50, 30.00);
  ...
  describe("eventId", () => {
    ...
    it("is unique across instances", () => {
      const a = new SellEvent("p1", "c1", "PETR4", 50, 30.00);
      const b = new SellEvent("p1", "c2", "VALE3", 30, 65.00);
  ...
  describe("edge cases", () => {
    it("handles zero quantity", () => {
      const event = new SellEvent("p1", "c1", "PETR4", 0, 30.00);
    ...
    it("handles fractional quantities", () => {
      const event = new SellEvent("p1", "c1", "PETR4", 10.5, 30.00);
