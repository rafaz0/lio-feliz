...
function makeOp(
  ...
  qty: number,
  price: number,
  overrides: Partial<Operation> & { fee?: number; irrf?: number; other_costs?: number } = {},
): Operation {
  return {
...
function L(positions: { ticker: string }[], ticker: string) {
  return positions.find((p) => p.ticker === ticker);
}
function N(positions: Position[], ticker: string) {
...
describe("Compra 100 PETR4 a 25.5 (sem fees)", () => {
  ...
  it("Legado: qty=100, invested=2550, avgPrice=25.5", () => {
    ...
    expect(p.invested).toBe(2550);
    expect(p.avgPrice).toBe(25.5);
  });
  it("Novo: qty=100, totalCost=2550, avgCost=25.5", () => {
    const p = N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!;
    expect(p.getQuantity().getValue()).toBe(100);
    expect(p.getTotalCost().getValue()).toBe(2550);
    expect(p.getAvgCost().getValue()).toBe(25.5);
  });
});
...
describe("Compra 100 PETR4 a 25 + fee 5 + IRRF 3", () => {
  ...
  it("Novo (com fee+irrf): totalCost=2508", () => {
    expect(
      N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!
        .getTotalCost()
        .getValue(),
    ).toBe(2508);
  });
...
describe("Compra + dividendo (sem fee)", () => {
  ...
  it("Legado: cost=1800", () => {
    expect(L(consolidatePortfolio(ops).positions, "PETR4")!.invested).toBe(1800);
  });
  it("Novo: cost=1800", () => {
    expect(
      N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!
...
describe("Compra + JCP (via metadata)", () => {
  ...
  it("Legado: cost=1850", () => {
    expect(L(consolidatePortfolio(ops).positions, "PETR4")!.invested).toBe(1850);
  });
  it("Novo: cost=1850 (via JcpEvent)", () => {
    expect(
      N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!
        .getTotalCost()
        .getValue(),
...
describe("Compra + bonus (via bonus_ratio)", () => {
  ...
  it("Legado: qty=110, cost=2000", () => {
    ...
    expect(p.invested).toBeCloseTo(2000, 0);
  });
  it("Novo: qty=110, cost=2000 (BonusEvent com sharesHeld=100/ratio=0.1)", () => {
    const p = N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!;
    expect(p.getQuantity().getValue()).toBe(110);
    expect(p.getTotalCost().getValue()).toBe(2000);
  });
});
...
describe("Venda total zera posicao", () => {
  ...
  it("Legado: exclui do resumo", () => {
    expect(consolidatePortfolio(ops).positions).toHaveLength(0);
  });
  it("Novo: qty=0, cost=0 (mantida)", () => {
    const p = N(new PortfolioProjector().project(opsToEvents(ops)), "PETR4")!;
    expect(p.getQuantity().getValue()).toBe(0);
    expect(p.getTotalCost().getValue()).toBe(0);
  });
});
...
describe("Split/Grouping/Amortization/Adjustment (soh no novo dominio)", () => {
  ...
  it("Novo: seq completa produz qty=1000, cost=7200", () => {
    ...
    expect(p.getQuantity().getValue()).toBe(1000);
    expect(p.getTotalCost().getValue()).toBe(7200);
  });
});
...
describe("Sequencia multi-evento COM fees (cenario realista)", () => {
  const ops: Operation[] = [
    ...
    makeOp("bonus", "PETR4", 7.5, 0, {
      fee: 0,
      traded_at: "2024-06-01T10:00:00Z",
      created_at: "2024-06-01T10:00:00Z",
      metadata: { bonus_ratio: 0.05 },
    }),
  ];
  ...
  it("VALE3: legado == novo (com fee na compra)", () => {
    ...
    expect(leg.invested).toBeCloseTo(9001.5, 1);
    expect(nov.getQuantity().getValue()).toBe(150);
    expect(nov.getTotalCost().getValue()).toBeCloseTo(9001.5, 1);
  });
...
describe("⚠️ DIVERGENCIA: PortfolioHistory — estruturas incompativeis", () => {
  ...
  it("Novo PortfolioHistoryCalculator: snapshots por evento SEM precos de mercado", () => {
    const h = new PortfolioHistoryCalculator().calculate(
      opsToEvents([makeOp("buy", "PETR4", 100, 25)]),
    );
