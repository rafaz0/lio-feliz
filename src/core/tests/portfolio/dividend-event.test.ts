...
describe("DividendEvent", () => {
  describe("creation", () => {
    it("creates a dividend event with given properties", () => {
      const event = new DividendEvent("portfolio-1", "corr-1", "PETR4", 100, 1.5);
      expect(event.aggregateId).toBe("portfolio-1");
      expect(event.correlationId).toBe("corr-1");
      expect(event.assetId).toBe("PETR4");
      expect(event.shares).toBe(100);
      expect(event.amountPerShare).toBe(1.5);
    });

    it("sets type to DIVIDEND", () => {
      const event = new DividendEvent("p1", "c1", "PETR4", 100, 1.5);
  ...
  describe("eventId", () => {
    ...
    it("is unique across instances", () => {
      const a = new DividendEvent("p1", "c1", "PETR4", 100, 1.5);
      const b = new DividendEvent("p1", "c2", "VALE3", 50, 2.0);
  ...
  describe("edge cases", () => {
    it("handles zero shares", () => {
      const event = new DividendEvent("p1", "c1", "PETR4", 0, 1.5);
    ...
    it("handles fractional shares", () => {
      const event = new DividendEvent("p1", "c1", "PETR4", 10.5, 1.5);
