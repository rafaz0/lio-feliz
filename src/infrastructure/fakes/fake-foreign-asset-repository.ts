...
export class FakeForeignAssetRepository implements IForeignAssetRepository {
  ...
  async saveAsset(asset: ForeignAsset): Promise<void> { this.assets.set(asset.id.value, asset); }
  async findAssetById(assetId: string): Promise<ForeignAsset | null> { return this.assets.get(assetId) ?? null; }
  async findAssetsByPortfolio(_portfolioId: string): Promise<ForeignAsset[]> { return Array.from(this.assets.values()); }
  async deleteAsset(assetId: string): Promise<void> { this.assets.delete(assetId); }

  async saveExchangeRate(rate: ExchangeRate): Promise<void> { this.rates.set(`${rate.ticker}:${rate.currency}`, rate); }
  ...
  reset(): void { this.assets.clear(); this.rates.clear(); }
}
