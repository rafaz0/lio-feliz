...
import { ShieldCheck, Wallet, PiggyBank, Calendar, TrendingUp, ArrowRight } from "lucide-react";
import {
  Area,
  AreaChart,
  CartesianGrid,
...
export const Route = createFileRoute("/_authenticated/carteira/analise")({
  head: () => ({
    meta: [
      { title: "Análise de Cobertura — Investidor Pro" },
      {
        name: "description",
        content: "Quanto os dividendos da sua carteira cobrem suas despesas mensais projetadas.",
      },
    ],
  }),
  component: AnaliseWrapper,
});

const EXPENSE_KEY = "lio.cobertura.despesas";

function AnaliseWrapper() {
  const fetchOps = useServerFn(listOperations);
  const fetchProj = useServerFn(getRealProjections);
  const opsQuery = useQuery({ queryKey: ["operations"], queryFn: () => fetchOps() });
  ...
  const byTicker = useMemo(() => {
    ...
    for (const p of projections) {
      if (!map[p.ticker]) map[p.ticker] = { ticker: p.ticker, name: p.name, total: 0 };
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
            <Wallet className="size-3.5" /> Despesas mensais alvo
          </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
            <PiggyBank className="size-3.5" /> Proventos / mês (projetado)
          </div>
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={monthlySeries} margin={{ left: 8, right: 8, top: 8, bottom: 0 }}>
              <defs>
                <linearGradient id="analiseFill" x1="0" y1="0" x2="0" y2="1">
        ...
        {projections.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {projections.slice(0, 8).map((p, i) => {
              const pct = annualProjection > 0 ? (p.amount / annualProjection) * 100 : 0;
              return (
