...
import {
  toGlossaryTermViewModel,
  type GlossaryTermViewModel,
} from "../viewmodels/education.view-model";
...
export function useGlossaryQuery(term: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<GlossaryTermDto> => {
      const r = await dispatcher.DispatchQuery<GlossaryTermDto>({
        type: "ObterTermoQuery",
        term,
      } as IQuery);
...
export function useSearchGlossaryQuery(query: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<GlossarySearchDto> => {
      const r = await dispatcher.DispatchQuery<GlossarySearchDto>({
        type: "BuscarGlossarioQuery",
        query,
      } as IQuery);
      if (r instanceof Error) throw r;
      return r;
