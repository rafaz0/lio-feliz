...
interface AlertsPageProps { userId: string }

export function AlertsPage({ userId }: AlertsPageProps) {
  const [tab, setTab] = useState<"pending" | "rules">("pending");
  const [showForm, setShowForm] = useState(false);

  const { data: alerts, isLoading: alertsLoading, isError: alertsError, refetch: refetchAlerts } = useAlertsQuery(userId);
  ...
  if (alertsError) return <AlertsError message="Erro ao carregar alertas" onRetry={refetchAlerts} />;
    ...
    <div data-testid="alerts-page" className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setTab("pending")} className={`rounded-md px-3 py-1 text-xs ${tab === "pending" ? "bg-foreground text-background" : "border"}`}>Pendentes</button>
          <button onClick={() => setTab("rules")} className={`rounded-md px-3 py-1 text-xs ${tab === "rules" ? "bg-foreground text-background" : "border"}`}>Regras</button>
        </div>
      </div>
        ...
        <>
          {!alerts || alerts.length === 0 ? <AlertsEmpty /> : (
            <div className="space-y-3">
              {alerts.map((alert) => (
                <AlertCard key={alert.id} alert={alert} onConfirm={(id) => confirmAlert.mutate(id)} isPending={confirmAlert.isPending} />
              ))}
            </div>
          )}
        </>
      ...
      {tab === "rules" && (
        <div className="space-y-4">
          <button onClick={() => setShowForm(!showForm)} className="rounded-md bg-foreground px-3 py-1 text-xs text-background">{showForm ? "Cancelar" : "Nova regra"}</button>
