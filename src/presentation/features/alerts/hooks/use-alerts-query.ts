...
import {
  toAlertViewModels,
  toAlertRuleViewModels,
  type AlertViewModel,
  type AlertRuleViewModel,
} from "../viewmodels/alert.view-model";
...
export function useAlertsQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AlertaDto[]> => {
      const r = await dispatcher.DispatchQuery<AlertaDto[]>({
        type: "ObterAlertaQuery",
        userId,
      } as IQuery);
...
export function useAlertRulesQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AlertRuleDto[]> => {
      const r = await dispatcher.DispatchQuery<AlertRuleDto[]>({
        type: "ListarAlertasAtivosQuery",
        userId,
      } as IQuery);
      if (r instanceof Error) throw r;
      return r;
