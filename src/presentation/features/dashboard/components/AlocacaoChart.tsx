...
import type { AlocacaoItemViewModel } from "../types/dashboard.view-model";

interface AlocacaoChartProps {
  alocacao: AlocacaoItemViewModel[];
}

export function AlocacaoChart({ alocacao }: AlocacaoChartProps) {
  const chartData = alocacao.map((item) => ({
    classe: item.classe,
  ...
  return (
    <Card data-testid="alocacao-chart">
      <CardHeader>
          ...
          {alocacao.map((item) => (
            <li key={item.classe} className="flex items-center justify-between text-sm">
              ...
              <span className="tabular-nums">
                {item.percentual.toFixed(1)}% · {item.valor}
              </span>
            </li>
          ))}
        </ul>
