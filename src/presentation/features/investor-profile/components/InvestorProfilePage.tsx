...
export function InvestorProfilePage({ userId }: InvestorProfilePageProps) {
  ...
  if (profileLoading || questionsLoading) {
    return <div data-testid="profile-loading" className="py-8 text-center text-sm text-muted-foreground">Carregando...</div>;
  }
    ...
    return (
      <div data-testid="investor-profile-page" className="space-y-6">
        ...
        <div className="rounded-lg border p-4">
          <p>Perfil: <strong>{profile.riskLevelLabel}</strong></p>
  ...
  if (!questions) {
    return <div data-testid="profile-empty" className="py-8 text-center text-sm text-muted-foreground">Nenhuma pergunta disponivel.</div>;
  }
  ...
  return (
    <div data-testid="investor-profile-page" className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">Responda as 8 perguntas para descobrir seu perfil de investidor.</p>
            ...
            <div className="space-y-1">
              {q.options.map((opt, optIdx) => (
                <label key={optIdx} className="flex items-center gap-2 text-sm">
                  <input type="radio" name={q.id} value={optIdx} checked={answers[q.id] === optIdx} onChange={() => handleAnswer(q.id, optIdx)} />
                  {opt}
                </label>
              ))}
            </div>
      ...
      <button onClick={handleSubmit} disabled={!allAnswered} className="rounded-md bg-foreground px-4 py-2 text-sm text-background disabled:opacity-50">
