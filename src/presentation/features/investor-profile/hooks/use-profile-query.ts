...
} from "../viewmodels/investor-profile.view-model";
...
export function useProfileQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<InvestidorPerfilCompletoDto> => {
      const r = await dispatcher.DispatchQuery<InvestidorPerfilCompletoDto>({
        type: "ObterPerfilQuery",
        userId,
      } as IQuery);
      if (r instanceof Error) throw r;
      return r;
    },
    select: (data): ProfileViewModel | null =>
      data ? toProfileViewModel(data.profile, data.lastResult) : null,
  });
}
...
export function useQuestionnaireQuery() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<QuestionarioPerguntasDto[]> => {
      const r = await dispatcher.DispatchQuery<QuestionarioPerguntasDto[]>({
        type: "ObterQuestionarioQuery",
      } as IQuery);
      if (r instanceof Error) throw r;
      return r;
    },
