...
interface BacktestsPageProps { userId: string }

export function BacktestsPage({ userId }: BacktestsPageProps) {
  const [selectedStrategyId, setSelectedStrategyId] = useState<string>("");
  const [showForm, setShowForm] = useState(false);

  const { data: strategies, isLoading: stratLoading, isError: stratError, refetch } = useStrategiesQuery(userId);
  ...
  if (stratError) return <BacktestsError message="Erro ao carregar estrategias" onRetry={refetch} />;

  return (
    <div data-testid="backtests-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Backtests</h1>
        <button onClick={() => setShowForm(!showForm)} className="rounded-md bg-foreground px-3 py-2 text-sm text-background">
            ...
            <div key={s.id} className="rounded-lg border p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {s.allocations.map((a) => `${a.assetTicker} ${a.weightPercentage}%`).join(" | ")} vs {s.benchmarkTicker}
                </p>
              </div>
              <button
                onClick={() => { setSelectedStrategyId(s.id); executeBacktest.mutate(s.id); }}
              ...
              >
                {executeBacktest.isPending && selectedStrategyId === s.id ? "Executando..." : "Executar"}
              </button>
            </div>
          ))}
        </div>
