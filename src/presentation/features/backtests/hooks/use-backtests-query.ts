...
import { toBacktestResultViewModel, type BacktestResultViewModel } from "../viewmodels/backtest.view-model";
...
export function useBacktestsQuery(backtestId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BacktestCompletoDto> => {
      const r = await dispatcher.DispatchQuery<BacktestCompletoDto>({ type: "ObterBacktestQuery", backtestId } as IQuery);
      if (r instanceof Error) throw r; return r;
    },
    select: (data): BacktestResultViewModel | null => data.result ? toBacktestResultViewModel(data.result) : null,
  });
}
