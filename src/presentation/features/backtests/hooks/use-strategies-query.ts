...
import type { ApplicationError, EstrategiaListDto } from "@/presentation/shared/types/application-layer";
...
export function useStrategiesQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<EstrategiaListDto> => {
      const r = await dispatcher.DispatchQuery<EstrategiaListDto>({ type: "ListarEstrategiasQuery", userId } as IQuery);
