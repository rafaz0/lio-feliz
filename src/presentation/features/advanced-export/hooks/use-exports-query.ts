...
import {
  toExportTemplateViewModels,
  toExportJobViewModels,
  type ExportTemplateViewModel,
  type ExportJobViewModel,
} from "../viewmodels/exports.view-model";
...
export function useExportTemplatesQuery() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ExportTemplateDto[]> => {
      const r = await dispatcher.DispatchQuery<ExportTemplateDto[]>({
        type: "ListarExportTemplatesQuery",
      } as IQuery);
...
export function useExportJobsQuery(portfolioId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ExportJobListDto> => {
      const r = await dispatcher.DispatchQuery<ExportJobListDto>({
        type: "ListarExportTemplatesQuery",
        portfolioId,
      } as IQuery);
      if (r instanceof Error) throw r;
