...
interface AdvancedExportPageProps { portfolioId: string }
...
export function AdvancedExportPage({ portfolioId }: AdvancedExportPageProps) {
  ...
  if (isError) return <AdvancedExportError message="Erro ao carregar templates" onRetry={refetch} />;

  return (
    <div data-testid="exports-page" className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold">Exportacao Avancada</h1>
        <p className="text-sm text-muted-foreground">Exporte relatorios em PDF, CSV, JSON ou XLSX.</p>
      </div>

      {!templates || templates.length === 0 ? <AdvancedExportEmpty /> : (
        <div className="space-y-3">
          {templates.map((t) => (
            <ExportTemplateCard key={t.id} template={t} onExport={(id) => solicitar.mutate(id)} isPending={solicitar.isPending} />
          ))}
        </div>
      )}
      ...
      {solicitar.data && (
        <div className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm">
          ...
          {solicitar.data.checksum && <p className="text-xs text-muted-foreground font-mono mt-1">Checksum: {solicitar.data.checksum}</p>}
        </div>
      )}
    </div>
  );
