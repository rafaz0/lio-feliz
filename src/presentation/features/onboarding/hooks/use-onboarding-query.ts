...
export function useOnboardingQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<OnboardingCompletoDto> => {
      const r = await dispatcher.DispatchQuery<OnboardingCompletoDto>({
        type: "ObterProgressoOnboardingQuery",
        userId,
      } as IQuery);
    ...
    select: (data): OnboardingViewModel => ({
      ...
      progressPercent:
        data.progress.totalSteps > 0
          ? Math.round((data.progress.currentStep / data.progress.totalSteps) * 100)
          : 0,
    }),
  });
}
