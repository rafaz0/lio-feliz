...
export function useAvancarPassoMutation(userId: string) {
  ...
  return useMutation({
    mutationFn: async (): Promise<OnboardingCompletoDto> => {
      const r = await dispatcher.DispatchCommand<OnboardingCompletoDto>({ type: "AvancarPassoCommand", userId } as unknown as ICommand);
      if (r instanceof Error) throw r; return r;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ONBOARDING_QUERY_KEYS.progress(userId) }),
  });
}
...
export function usePularOnboardingMutation(userId: string) {
  ...
  return useMutation({
    mutationFn: async (): Promise<OnboardingCompletoDto> => {
      const r = await dispatcher.DispatchCommand<OnboardingCompletoDto>({ type: "PularOnboardingCommand", userId } as unknown as ICommand);
      if (r instanceof Error) throw r; return r;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ONBOARDING_QUERY_KEYS.progress(userId) }),
  });
