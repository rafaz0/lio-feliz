...
import type { ApplicationError, PlanoDetalhadoDto, AssinaturaDto } from "@/presentation/shared/types/application-layer";
import { SUBSCRIPTION_QUERY_KEYS } from "../queries";
import { toPlanViewModels, toSubscriptionViewModel, type PlanViewModel, type SubscriptionViewModel } from "../viewmodels/subscription.view-model";
...
export function usePlansQuery() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PlanoDetalhadoDto[]> => {
      const r = await dispatcher.DispatchQuery<PlanoDetalhadoDto[]>({ type: "ListarPlanosQuery" } as IQuery);
...
export function useSubscriptionQuery(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AssinaturaDto> => {
      const r = await dispatcher.DispatchQuery<AssinaturaDto>({ type: "ObterPlanoAtivoQuery", userId } as IQuery);
      if (r instanceof Error) throw r; return r;
    },
